...
You are strictly forbidden from writing scripts in `tools/` until:
    - Discovery Questions are answered (Confirmed by Luxury Prompt)
    - The Data Schema is defined in `gemini.md`
    - `task_plan.md` has an approved Blueprint

---

## 💎 Project Constitution: Luxury AI SaaS Website

### 1. Data Schema (Payload Shapes)

```json
{
  "project": "Luxury AI SaaS",
  "theme": {
    "colors": {
      "primary": "#6C5CE7",
      "secondary": "#8F85F3",
...
### 2. Behavioral Rules (The "Luxury" Invariants)
- **Negative Space**: Minimum 80px vertical padding between sections.
- **Micro-animations**: All transitions must be 0.4s ease-out.
- **Glassmorphism**: Use `backdrop-filter: blur(10px)` for navbar and cards.
- **No Emojis**: Use Lucide icons exclusively as per `ui-ux-pro-max` skill.
- **Typography**: Headers must have `letter-spacing: -0.02em`.
